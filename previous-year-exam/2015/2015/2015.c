...
int statistic(Tree *root, char toytarget[]){
    ...
    if (strcmp(root->toy, toytarget) == 0) return 1+statistic(root->left, toytarget)+statistic(root->right, toytarget);
    else return statistic(root->left, toytarget)+statistic(root->right, toytarget);
}
...
void func5(){
    ...
    printf("\nTHONG KE\n");
    int i;
    printf("%s\t%d\n","maybay",statistic(RA,"maybay"));
    printf("%s\t%d\n","dientu",statistic(RA,"dientu"));
    printf("%s\t%d\n","oto",statistic(RA,"oto"));
    printf("%s\t%d\n","gaubong",statistic(RA,"gaubong"));
